...
function addCommonFoodToBloodRecipe(event,item,value,name){
    ...
    .id(name)
    .perTick(builder => builder.inputFE(120))
    .duration(200)
    .inputItems(item)
...
function getFoodBlood(a,b){
    var value = Math.round(a * 30 * (1 + b * 0.33))
    return value == 0 ? 1 : value
}

ServerEvents.recipes((event) => {
    BxP2.getAllBXItems().forEach((k,v) => {
        addSimplePowerRecipe(event,k,Math.round(v * 125),BxP2.getRecipeName(k,"bx_simple_power"))
        addSimpleManaRecipe(event,k,Math.round(v * 8),BxP2.getRecipeName(k,"bx_simple_mana"))
    })
